...
export default function AutomationsPage() {
  ...
  function daysLeft(deactivatesAt: string | null) {
    if (!deactivatesAt) return null
    const diff = Math.ceil((new Date(deactivatesAt).getTime() - Date.now()) / (1000 * 60 * 60 * 24))
    return diff
  }
      ...
      {automations.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {automations.map(automation => {
            const days = daysLeft(automation.deactivates_at)
            const isExpiringSoon = days !== null && days <= 2 && days > 0
            const isExpired = days !== null && days <= 0

            return (
              <div key={automation.id}
                style={{ background: 'var(--canvas)', borderRadius: 'var(--radius-md)', border: `1px solid ${isExpiringSoon ? '#fbbf24' : isExpired ? '#fca5a5' : 'var(--hairline)'}`, padding: '20px 24px', transition: 'border-color 0.15s' }}
                onMouseEnter={e => { if (!isExpiringSoon && !isExpired) e.currentTarget.style.borderColor = 'var(--stone)' }}
                onMouseLeave={e => { if (!isExpiringSoon && !isExpired) e.currentTarget.style.borderColor = 'var(--hairline)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  ...
                  {automation.post_thumbnail_url && !automation.post_thumbnail_url.includes('placeholder') ? (
                    ...
                    <div style={{ width: 52, height: 52, borderRadius: 'var(--radius-sm)', background: 'var(--card)', border: '1px solid var(--hairline)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 20 }}>📷</div>
                  )}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                      ...
                      {/* Auto-deactivate badge */}
                      {days !== null && days > 0 && (
                        <span style={{ background: isExpiringSoon ? '#fff8e0' : 'var(--card)', borderRadius: 'var(--radius-full)', padding: '2px 10px', fontSize: 11, fontWeight: 700, color: isExpiringSoon ? '#92610a' : 'var(--ash)' }}>
                          {isExpiringSoon ? `⚠ Pauses in ${days}d` : `Auto-pauses in ${days}d`}
                        </span>
                      )}
                      {isExpired && (
                        <span style={{ background: '#fff0f0', borderRadius: 'var(--radius-full)', padding: '2px 10px', fontSize: 11, fontWeight: 700, color: 'var(--red)' }}>
                          Auto-paused
                        </span>
                      )}
                    </div>
                    ...
                    {automation.trigger_type === 'keyword' && automation.keywords && (
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 6 }}>
                        {automation.keywords.map((kw: string) => (
                          <span key={kw} style={{ background: 'var(--ink)', color: '#fff', borderRadius: 'var(--radius-full)', padding: '2px 10px', fontSize: 11, fontWeight: 700 }}>{kw}</span>
                        ))}
                      </div>
                    )}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <button onClick={() => toggleAutomation(automation.id, automation.is_active)}
                        style={{
                          ...
                          background: automation.is_active ? 'var(--ink)' : 'var(--stone)',
                      ...
                      <span style={{ fontSize: 11, color: 'var(--ash)', width: 38 }}>
                        {automation.is_active ? 'Active' : 'Paused'}
                      </span>
                    </div>

                    {/* Edit button */}
                    <Link href={`/dashboard/automations/edit/${automation.id}`}
                      style={{ background: 'var(--card)', border: '1px solid var(--hairline)', borderRadius: 'var(--radius-sm)', padding: '6px 12px', fontSize: 12, fontWeight: 700, color: 'var(--mute)', cursor: 'pointer', textDecoration: 'none', transition: 'all 0.15s' }}
                      onMouseEnter={e => { e.currentTarget.style.background = 'var(--secondary-bg)'; e.currentTarget.style.color = 'var(--ink)' }}
                      onMouseLeave={e => { e.currentTarget.style.background = 'var(--card)'; e.currentTarget.style.color = 'var(--mute)' }}>
                      Edit
                    </Link>

                    {/* Delete button */}
                    <button onClick={() => deleteAutomation(automation.id)}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, color: 'var(--ash)', fontWeight: 600, padding: '6px 10px', borderRadius: 'var(--radius-sm)', transition: 'all 0.15s' }}
